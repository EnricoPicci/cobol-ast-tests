      ******************************************************************
      * SAFE02-CALLER.cob — Caller Module for Correct Oracle Pattern
      *
      * PURPOSE:
      *   This is the CALLER in a two-module example showing the
      *   CORRECT way to handle Oracle host variables when compiled
      *   with BINARY(BE).
      *
      *   The caller is identical to ENDIAN02-CALLER.cob — it
      *   defines parameters as COMP (big-endian under BINARY(BE)),
      *   which is correct for MQ and inter-module communication.
      *
      *   The difference is entirely in the CALLED module
      *   (SAFE02-CALLED.cob), which properly converts COMP to
      *   COMP-5 before passing values to Oracle.
      *
      ...
      *> COMP under BINARY(BE) = big-endian, matching MQ/AIX.
      ...
      *> Parameters are COMP (big-endian) — the called module
      *> will convert to COMP-5 before using them with Oracle.
